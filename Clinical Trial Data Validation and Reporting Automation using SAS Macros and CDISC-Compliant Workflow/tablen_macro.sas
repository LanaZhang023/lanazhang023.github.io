/*--- Logs: -------------------------------------------------------------------
 ...
 08/18/2025:
  1). Downloaded from: https://communities.sas.com/t5/SAS-Communities-Library/
Demographic-Table-and-Subgroup-Summary-Macro-TABLEN/ta-p/634030
  2). Test it: can put multiple macro run to a single trf file;
  3). Added RTF code in the second Proc Report to add border line to bottom
of the super header and bold the header name;
  
  
  Modified by: Yingchen (Lana) Zhang
-----------------------------------------------------------------------------*/
    ...
    %if &_nsurvvars_>0 %then %do;
        ...
        %if &_anytimelist=1 %then %do;
            data _surv_stat_tl2;
                ...
                /* changed 'Est' to 'Survival': */
                factor=catx(' ',timelist,'-',scan("%superq(time_units)",_survvar_,'|','m'),'Survival (95% CI)');
                n_pct=tl;
                drop timelist tl;
            run;
        %end;
                    ...
                    %end;
                        %do c = 1 %to &ncolby;
                           ...
                           %end;   
                               ...
                               %if %sysevalf(%superq(by)^=,boolean) %then %do;
                                    %if %sysevalf(%superq(showbylabel)=1,boolean) %then %do;
                                    /* want to add bottom border here, but not working, but now working here,
                                    working if do it in next Proc Report: */
                                    ("&bylabel"
                                    %end;
                                    c&c._m_npct 
                               %end;
                        ...
                        %end;
                            %do c = 1 %to &ncolby;
                               ...
                               %end;   
                                   ...
                                   %if %sysevalf(%superq(by)^=,boolean) %then %do;
                                        %if %sysevalf(%superq(showbylabel)=1,boolean) %then %do;
                                        /* Want to add border line to bottom _ZChen */
                                       /* but this would not work for trf file, nore html, not sure for pdf: */
                                        /*^S={borderbottomcolor=black   
                                    borderbottomwidth=.5pt borderrightcolor=black  
                                    borderrightwidth =.5pt borderleftcolor =cxD9D9D9   
                                    borderleftwidth=.5pt cellheight=.23in} */
                                    /* used rtf code here will work for rtf file, but in html, would not see it. 
                                    Also want bold the header:*/
                                        ("(*ESC*)R'\brdrb\brdrs\brdrw26' (*ESC*)S={font_weight=bold} &bylabel" 
                                        %end;
                                        c&c._m_npct 
                                   %end;
                                       /*Print result variables*/
